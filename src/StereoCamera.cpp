...
vector<Point2f> StereoCamera::TrackFeatures_Time(const Mat &image,const Mat &image_new, const vector<Point2f> &lm)
{
...
vector<Point2f> StereoCamera::TrackFeatures_LR(const Mat &image_left, const Mat &image_right, const vector<Point2f> &landmarksleft)
{
...
void StereoCamera::Triangulation_Euroc(const vector<Point2f> &pl, const vector<Point2f> &pr, vector<Point3f> &p_3d)
{
    int N = pl.size();
    Eigen::Matrix4d R1;
...
vector<int> StereoCamera::GenerateDiffNumber(int min,int max,int num)
{
    int rnd;
    vector<int> diff;
    vector<int> tmp;
...
void StereoCamera::Save_Point(vector<Point3f>& vs, const string file)
{
